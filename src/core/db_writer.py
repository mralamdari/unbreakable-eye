...
def _dispatch(typ: str, cmd: tuple, response_queues: dict) -> None:
    ...
    if typ == "update_customer_last_seen":
        ...
        logger.info(f"DB writer: update_customer_last_seen {customer_id}")
        ...
        logger.info(f"DB writer: store_embedding for customer {customer_id}, cam {cam_id}")
        with write_connection() as conn:
        ...
        logger.info(f"DB writer: match_or_register for cam {cam_id}, tracker {track_id}")
