...
#         self.frame_shape  = settings.FRAME_SHAPE    # (512, 512, 3) — inference
#         self.frame_bytes  = settings.FRAME_BYTES
#         self.native_shape = settings.NATIVE_SHAPE   # (1080, 1920, 3) — display
#         self.native_bytes = settings.NATIVE_BYTES

#         # Ring buffer for inference (small, 4 slots)
#         self.input_shm = shared_memory.SharedMemory(
#             create=True,
#             size=self.frame_bytes * buffer_slots)
#         self.input_shm_name = self.input_shm.name

#         # Single-slot buffer for full-res frames (display + crop extraction)
#         # One slot — embedder always reads latest, reader always writes latest
#         self.display_shm = shared_memory.SharedMemory(
#             create=True,
#             size=self.native_bytes)
#         self.display_shm_name = self.display_shm.name

#         # Output SHM — annotated full-res frame for MJPEG streaming
#         self.output_shm = shared_memory.SharedMemory(
#             create=True,
#             size=self.native_bytes)
#         self.output_shm_name = self.output_shm.name
...
#                 self.cam_id,
#                 self.input_shm_name,
#                 self.display_shm_name,   # ← new
#                 self.frame_shape,
#                 self.frame_bytes,
#                 self.native_shape,       # ← new
#                 self.native_bytes,       # ← new
#                 self.free_slots,
#                 self.frame_ready_queue,
#                 self.det_queue,
#                 self.stop_event,
...
#             args=(
#                 self.input_shm_name,
#                 self.display_shm_name,   # ← replaces output_shm as crop source
#                 self.output_shm_name,    # ← still used for annotated output
#                 self.frame_shape,
#                 self.frame_bytes,
#                 self.native_shape,       # ← new
#                 self.native_bytes,       # ← new
...
#         self.input_shm.close();   self.input_shm.unlink()
#         self.display_shm.close(); self.display_shm.unlink()
#         self.output_shm.close();  self.output_shm.unlink()

#     def get_latest_frame(self):
#         frame = np.ndarray(
#             self.native_shape,       # ← native resolution now
#             dtype=np.uint8,
#             buffer=self.output_shm.buf
#         )
#         return frame.copy()
...
#             if tracker_id in track_to_customer:
#                 # ── Known tracker: check diversity, maybe store new exemplar ──
#                 customer_id = track_to_customer[tracker_id]
#                 request_id = f"dist_{cam_id}_{tracker_id}_{time.time()}"
#                 db_queue.put((
#                     "min_dist_to_customer",
...
#             else:
#                 # ── Unknown tracker: match against DB or register as new ──
#                 request_id = f"match_{cam_id}_{tracker_id}_{time.time()}"
#                 db_queue.put((
#                     "match_or_register",
#                     emb, cam_id, time.time(), request_id,
...
#                 # customer_id is now known — came back from db_writer
#                 track_to_customer[tracker_id] = customer_id
...
#             # customer_id is always defined here — either from track_to_customer
#             # (known path) or from db_writer reply (unknown path)
...
# # As soon as a frame is read successfully, consecutive_failures resets to 0.
# # During the wait, the thread is sleeping – it consumes almost zero CPU.
# def reader_worker(rtsp_url, cam_id,
#                   shm_name,         # inference ring buffer
#                   display_shm_name, # full-res single slot
#                   frame_shape, frame_bytes,
#                   native_shape, native_bytes,
#                   free_slots, frame_ready_queue, det_queue, stop_event):

#     pin_process([0])
#     shm         = shared_memory.SharedMemory(name=shm_name)
#     display_shm = shared_memory.SharedMemory(name=display_shm_name)
...
#                 cap.release()
#                 cap = None
#             sleep_time = 0 if (first_attempt and consecutive_failures < 6) \
#                          else min(1.0 * (2 ** consecutive_failures), max_backoff)
#             first_attempt = False
#             stop_event.wait(sleep_time)
#             if stop_event.is_set():
#                 break
...
#         # Always write full-res frame to display_shm — embedder reads it for crops
#         # Resize to native_shape if camera native differs from settings
#         if frame.shape[:2] != (native_shape[0], native_shape[1]):
#             display_frame = cv2.resize(
#                 frame, (native_shape[1], native_shape[0]),
#                 interpolation=cv2.INTER_LINEAR)
#         else:
#             display_frame = frame

#         dst = np.ndarray(native_shape, dtype=np.uint8, buffer=display_shm.buf)
#         np.copyto(dst, display_frame)

#         if frame_counter % 3 == 0:
#             # Keyframe — resize to inference resolution, write to ring buffer
#             try:
#                 idx = free_slots.get_nowait()
#             except queue.Empty:
#                 continue
...
# def embedder_worker(
#     input_shm_name,
#     display_shm_name,   # full-res source for crops and annotation
#     output_shm_name,    # full-res annotated output for streaming
#     frame_shape, frame_bytes,
#     native_shape, native_bytes,
#     det_queue, free_slots, stop_event,
#     db_queue, response_queue, cam_id
# ):
#     pin_process([4, 5])
#     input_shm   = shared_memory.SharedMemory(name=input_shm_name)
#     display_shm = shared_memory.SharedMemory(name=display_shm_name)
...
#         if idx is None:
#             # Skipped frame — read last display frame for re-annotation
#             frame = np.ndarray(
#                 native_shape, dtype=np.uint8, buffer=display_shm.buf).copy()
#             detections    = last_valid_detections
#             run_embedding = False
#         else:
#             # Real frame — read full-res from display_shm for crops/annotation
#             # Read inference-res from input_shm only to free the slot
#             frame = np.ndarray(
#                 native_shape, dtype=np.uint8, buffer=display_shm.buf).copy()
#             free_slots.put_nowait(idx)   # free inference slot immediately

#             if xyxy is not None:
#                 # Scale detection boxes from inference res → native res
#                 scale_x = native_shape[1] / frame_shape[1]  # width scale
#                 scale_y = native_shape[0] / frame_shape[0]  # height scale
#                 scaled_xyxy = xyxy.copy()
#                 scaled_xyxy[:, 0] *= scale_x  # x1
#                 scaled_xyxy[:, 1] *= scale_y  # y1
#                 scaled_xyxy[:, 2] *= scale_x  # x2
#                 scaled_xyxy[:, 3] *= scale_y  # y2
#                 detections = sv.Detections(
#                     xyxy=scaled_xyxy,
...
#         # process_frame now receives native-res frame with scaled boxes
#         annotated = process_frame(
#             frame=frame,
#             detections=detections,
#             tracker=tracker,
...
#         # Write annotated full-res frame to output_shm
#         output_frame = np.ndarray(
#             native_shape, dtype=np.uint8, buffer=output_shm.buf)
#         np.copyto(output_frame, annotated)
