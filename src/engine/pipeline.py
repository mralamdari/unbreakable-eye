...
###This is the deployment phase that can add some features and also is SUITABLE FOR READING THE VIDOE  FILES NOT RTSP URLS       
...
def reader_worker(rtsp_url,
    ...
    # Video FILES decode as fast as the CPU/disk allow — with nothing to pace
    # them, cap.read() rips through a recording in a few seconds ("plays
    # fast") and floods the ring buffer faster than the detector can keep
    # up, which drops far more frames than a live camera at the same
    # nominal fps would. RTSP/HTTP streams already arrive paced in real
    # time, so this only kicks in for local files.
    is_file_source = os.path.isfile(rtsp_url)
    native_fps = None
    frame_interval = None
    next_frame_due = None
    ...
    while not stop_event.is_set():
        if not online:
            ...
            if is_file_source:
                ...
                # Guard against corrupt/missing metadata (0, NaN, or absurd values).
                native_fps = fps_meta if fps_meta and 1 <= fps_meta <= 240 else 25.0
                frame_interval = 1.0 / native_fps
                next_frame_due = time.monotonic()
                logger.info(
                    f"Camera {cam_id}: file source ({rtsp_url}) — "
                    f"pacing playback at {native_fps:.2f} fps"
                )
        ...
        if is_file_source and next_frame_due is not None:
            now_mono = time.monotonic()
            # If we've fallen far behind schedule (e.g. the detector stalled
            # us for a while), don't blast through the backlog trying to
            # catch up in real time — just resume pacing from now.
            if now_mono - next_frame_due > 1.0:
                next_frame_due = now_mono
            sleep_for = next_frame_due - now_mono
            if sleep_for > 0:
